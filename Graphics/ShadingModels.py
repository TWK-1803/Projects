...
def generateEdgeTable(prevpoly, prevpoly2, poly, poly2, nextpoly, nextpoly2):
    ...
    while len(polyPoints) - len(visited) >= 2:
        ...
        Istart1 = getIntensity(
            Ia, Ip, Kd, Ks, SpecIndex, vertexNormals[indx], normalize(L), normalize(V)
        )
        Iend1 = getIntensity(
            Ia,
            Ip,
            ...
            Ks,
            SpecIndex,
            vertexNormals[(indx + 1) % len(vertexNormals)],
            normalize(L),
            normalize(V),
        )
...
def fillPoly(mode, prevpoly, poly, nextpoly, edgeColor, faceColor):
    ...
    if mode == 4:
        ...
        intensity = getIntensity(
            Ia, Ip, Kd, Ks, SpecIndex, N, normalize(L), normalize(V)
        )
    ...
    # 3 is faceColor, 4 is flat shading, 5 is Gouraud, 6 is Phong
        ...
        edgeTable = generateEdgeTable(
            prevcoords, prevpoly, displaycoords, poly, nextcoords, nextpoly
        )
        # print(edgeTable, end = "\n\n")
